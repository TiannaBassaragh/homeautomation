<template>
    <v-container fluid>
        <v-row class="row" justify = 'center'>
            <v-col class="columnBorder" cols="2" align="center">
                <v-sheet>
            ...
            <v-col>
                <v-sheet>
                    <v-card class="text-secondary" color="surface" variant="tonal">
                        <figure class="highcharts-figure, columnBorder">
        ...
        <v-row max-width: 1200px justify = 'center'>
            <v-col class="columnBorder" cols="7" align="left">
                <v-sheet>
                    <v-card class="text-secondary" color="surface" variant="tonal">
                        <figure class="highcharts-figure, columnBorder">
  ...
  import { useRoute, useRouter } from "vue-router";
    ...
    var start = ref(null);
    var end = ref(null);
    const areawaterHiLine = ref(null);
    const gaugewaterHiLine = ref(null);
    var fluidmeter = new FluidMeter();
  ...
  watch(payload, (data) => {
      ...
      if (data.waterheight >= 77) {
        ...
        fluidmeter.setPercentage(data.percentage.toFixed(2));
        areawaterHiLine.value.series[0].addPoint({ y: parseFloat(data.waterheight.toFixed(2)), x: data.timestamp*1000  }, true, shift.values); // Add new data point
